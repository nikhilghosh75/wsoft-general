/* The main script for the destruction library sourced from mjholtzem.
* Call fragment() to destroy the mesh or sprite at runtime.
* @Robert Keller (RKIV#9137)
*/
using UnityEngine;
...
public class Breakable : MonoBehaviour
{
    ...
    /// Creates fragments if necessary and destroys original gameobject
    /// </summary>
    public void fragment(Vector2 force)
    {
        //if fragments were not created before runtime then create them now
        if (fragments.Count == 0 && allowRuntimeFragmentation)
        {
        ...
        else
        {
            foreach (GameObject frag in fragments)
            {
                ...
                randomVector *= force.magnitude/10;
                frag.GetComponent<Rigidbody2D>().AddForce(force + randomVector, ForceMode2D.Impulse);
            }
        }
        //if fragments exist destroy the original
        if (fragments.Count > 0)
